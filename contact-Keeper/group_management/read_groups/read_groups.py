...
def read_groups(data):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            query = """
            SELECT m.role, m.title, m.notes, m.created_at, m.status, g.name, g.description FROM `meets` m LEFT JOIN `group_members` g ON m.group_id = g.id WHERE m.person_id = %s
            """
